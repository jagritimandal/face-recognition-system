from tkinter import Label,Button,Tk,StringVar,Frame,RIDGE,LabelFrame,simpledialog
...
class Student:
    def __init__(self,root):
        ...
        #title
        title_lbl=Label(text="STUDENT Details SYSTEM",font=("times new roman",15,"bold"),bg="darkblue",fg="whitesmoke")
        title_lbl.place(x=0,y=1,width=330,height=35)

        #back to main button4
        b1_1=Button(text="main page",cursor="hand2",command=self.go_to_main,font=("times new roman",10,"bold"),bg="skyblue",fg="black")
        b1_1.place(x=1000,y=5,width=200,height=40)
        ...
        update_photo_btn=Button(btn_photo_frame,text="Train dataset",width=35,font=("times new roman",12,"bold"),bg="blue",fg="white")
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department"or self.var_name.get()==""or self.var_Student_id.get()=="":
            ...
            try:
                ...
                # Function to capture and save photos for a student
                def capture_photos(student_id):
                    count = 0
                    student_dir = os.path.join(dataset_path, str(student_id))
                    ...
                    while True:
                        ...
                        # Capture photo when 'q' key is pressed
                        if key == ord('q'):
                            count += 1
                            filename = os.path.join(student_dir, f"{student_id}_{count}.jpg")
                            cv2.imwrite(filename, frame)
                            messagebox.showinfo("Photo", "Photo capture completed!")
                ...
                while True:
                    ...
                    # Capture photos for the entered student ID
                    capture_photos(student_id)
                    
                    # Ask if user wants to capture another student's photos
                    another = simpledialog.askstring("Input", "Do you want to capture another student's photos? (yes/no)")
